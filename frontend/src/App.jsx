...
export default function App() {
  ...
  const activeFilters = useMemo(() => {
    ...
    // Support either `tag` (array) or `tags` (array) depending on what ended up in query
    const tagArray =
      (Array.isArray(query.tag) && query.tag) ||
      (Array.isArray(query.tags) && query.tags) ||
      [];
    if (tagArray.length) f.push(`tags:${tagArray.join("|")}`);

    return f;
  }, [query]);

  // Map FilterBar's `tags` -> API's expected `tag` (list)
  const onApplyFilters = (params) => {
    const { tags, ...rest } = params || {};
    const next = { ...rest, page: 1 };
    if (Array.isArray(tags) && tags.length) next.tag = tags; // Flask uses request.args.getlist("tag")
    fetchJobs(next);
  };
  ...
  const onReset = () =>
    fetchJobs({
      ...
      sort: "date_desc",
      page: 1,
      // intentionally omit `tag` to clear tags
    });
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6">
      ...
      <div className="mt-6 flex items-center justify-center gap-2">
        <button
          ...
          onClick={() => fetchJobs({ page: data.page - 1 })}
        ...
        <button
          disabled={(data.items?.length || 0) < (data.page_size || 12)}
          onClick={() => fetchJobs({ page: data.page + 1 })}
          className="px-3 py-1 rounded border disabled:opacity-50"
        >
          Next
